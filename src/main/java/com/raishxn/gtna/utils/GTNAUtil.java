...
public class GTNAUtil {
    ...
    public static Tag serializeNBT(GTRecipe recipe) {
        ...
        tag.put("recipe",
                (Tag) GTRecipeSerializer.CODEC.encodeStart(NbtOps.INSTANCE, recipe).result().orElse(new CompoundTag()));
        return tag;
    }
    ...
    public static @Nullable GTRecipe deserializeNBT(Tag tag) {
        if (tag instanceof CompoundTag ctag) {
            ...
            GTRecipe recipe = (GTRecipe) GTRecipeSerializer.CODEC.parse(NbtOps.INSTANCE, ctag.get("recipe")).result()
                    .orElse((GTRecipe) null);
