...
public class IndustrialSlaughterhouse extends WorkableElectricMultiblockMachine
    ...
    public void addDisplayText(@NotNull List<Component> textList) {
        ...
        MultiblockDisplayText.builder(textList, isFormed())
                ...
                .addCustom(text -> {
                    text.add(
                            Component.literal("Mode: ").append(Component.literal(mode).withStyle(ChatFormatting.AQUA)));

                    if (tier >= base) {
                        int currentMult = (int) Math.pow(mBase, (tier - base));
                        text.add(Component.literal("Drop Mult: ")
                                .append(Component.literal(currentMult + "x").withStyle(ChatFormatting.GOLD)));
                    } else {
