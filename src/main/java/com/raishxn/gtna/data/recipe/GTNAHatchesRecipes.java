...
public class GTNAHatchesRecipes {
    ...
    public static void register(Consumer<FinishedRecipe> provider) {
        ...
        if (GTNAMachines2.THREAD_HATCHES[ZPM] != null) {
            ShapedRecipeBuilder.shaped(RecipeCategory.MISC, GTNAMachines2.THREAD_HATCHES[ZPM].asStack().getItem())
                    ...
                    .define('E', ChemicalHelper.get(TagPrefix.cableGtQuadruple, GTMaterials.UraniumRhodiumDinaquadide).getItem())
                    .define('F', GTMachines.HULL[ZPM].asStack().getItem())
                    .unlockedBy("has_hull_zpm", InventoryChangeTrigger.TriggerInstance.hasItems(GTMachines.HULL[ZPM].asStack().getItem()))
                    .save(provider);
        }
        ...
        if (GTNAMachines2.THREAD_HATCHES[UV] != null) {
            ShapedRecipeBuilder.shaped(RecipeCategory.MISC, GTNAMachines2.THREAD_HATCHES[UV].asStack().getItem())
                    ...
                    .define('E', ChemicalHelper.get(TagPrefix.cableGtQuadruple, GTMaterials.EnrichedNaquadahTriniumEuropiumDuranide).getItem())
                    .define('F', GTMachines.HULL[UV].asStack().getItem())
                    .unlockedBy("has_hull_uv", InventoryChangeTrigger.TriggerInstance.hasItems(GTMachines.HULL[UV].asStack().getItem()))
                    .save(provider);
        }
    ...
    private static void createAccelerateRecipe(Consumer<FinishedRecipe> provider, int tier, ItemLike sensor, ItemLike middleItem) {
    ...
    private static void createOverclockRecipe(Consumer<FinishedRecipe> provider, int tier, ItemLike fieldGen, ItemLike coil) {
